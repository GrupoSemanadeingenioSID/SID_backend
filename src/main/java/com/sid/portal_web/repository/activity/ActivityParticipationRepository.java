...
public interface ActivityParticipationRepository extends JpaRepository<ActivityParticipationEntity,Integer> {
    ...
    @Query("SELECT p FROM ActivityParticipationEntity p WHERE p.activity.activityId = :activityId")
    List<ActivityParticipationEntity> findByActivity_ActivityId(@Param("activityId") Integer activityId);
